...
async function getAvailableMechanics(serviceId, date, time) {
    ...
    const [serviceHours, serviceMinutes] = service.duration.split(":").map(Number);
    const endTime = new Date(startTime);
    endTime.setHours(startTime.getHours() + serviceHours, startTime.getMinutes() + serviceMinutes);
    ...
    for (let appointment of sameDayAppointments) {
        ...
        for (let repair of appointment.repairs) {
            ...
            if (!repairService || !repairService.duration) {
                console.log("Service de réparation introuvable ou durée non définie :", repair.service);
                continue; 
            }

            const [repairHours, repairMinutes] = repairService.duration.split(":").map(Number);
            const appointmentEndTime = new Date(appointmentStartTime);
            appointmentEndTime.setHours(appointmentStartTime.getHours() + repairHours, appointmentStartTime.getMinutes() + repairMinutes);
